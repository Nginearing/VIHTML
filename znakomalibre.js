(function () {
  ...
  form.addEventListener("submit", async function (event) {
    ...
    if (response.ok) {
      ...
      if (response.status === 401) {
        errorContainer.innerHTML = "tis jo, du humbadan spor.";
      } else {
  ...
  function guestbooks___populateQuestionChallenge() {
    const challengeQuestion = "ka 9 au 10?";
    const challengeHint = "blogeta flierezeriso";
  ...
  function guestbooks___loadMessages(reset) {
    ...
    var apiUrl =
      "https://guestbooks.meadow.cafe/api/v2/get-guestbook-messages/1074?page=" + currentPage + "&limit=" + pageSize;
    fetch(apiUrl)
      ...
      .then(function (data) {
        ...
        if (messages.length === 0 && currentPage === 1) {
          messagesContainer.innerHTML = "<p>jam nil neo posta.</p>";
        } else {
